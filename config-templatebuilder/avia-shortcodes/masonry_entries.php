			...
			foreach($this->loop as $entry)
			{
				...
				if($this->atts['caption_elements'] != 'none' || !empty($text_add))
				{
					if(strpos($this->atts['caption_elements'], 'title') !== false){
						...
						$items .= 	"<section class='av-masonry-entry-metadata'><h3 class='av-masonry-entry-title entry-title' {$markup}>{$the_title}</h3><p class='av-masonry-entry-date entry-date' {$markup}>{$date}</p></section>";
					}
				}
				...
				if($this->atts['caption_elements'] != 'none' || !empty($text_add))
				{
					...
					if(strpos($this->atts['caption_elements'], 'excerpt') !== false && !empty($content)){
						...
						$items .=	"<div class='av-masonry-entry-content entry-content' {$markup}><p>{$content}</p></div>";
					}
